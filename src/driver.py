...
class SparkDriver:
    ...
    def assign_task(self, task):
        ...
        self.task_node_table[task.task_id]=worker_info
    ...
    def init_tasks(self, lineage):
        ...
        for rdd, rdd_id in lineage:
            if isinstance(rdd, WideRDD):
                tasks += self.gen_stage_tasks(prev, stage_start, cur_stage_id)
                cur_stage_id += 1
                stage_start = rdd
            prev = rdd
    ...
    def gen_stage_tasks(self, start_rdd, last_rdd, cur_stage_id):
        ...
        for cur_par_id in range(0, len(graph_table)):
            if isinstance(start_rdd, TextFile):
                    ...
                    for tar_list in parent_graph:
                        for elem in tar_list:
                            if int(elem.split('_')[1]) == cur_par_id:
                                # elem_dict: {'parent_id': id, 'partition_id': id, worker_addr: 'XXXX:XX'}
                ...
                input_source = None
            tasks.update(Task(last_rdd, input_source, "{0}_{1}".format(cur_stage_id, cur_par_id)))
        return tasks
