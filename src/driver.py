...
class SparkDriver:
    ...
    def do_drive(self, last_rdd, action_name, *args):
        ...
        debug_print("[SparkDriver]*** Lineage:{0}".format(map(lambda x: x[1], lineage)),self._master.debug)
        debug_print("[SparkDriver]*** Partition Form:",self._master.debug)
        for rdd in lineage:
            debug_print("[SparkDriver]***   {0}".format(rdd[0].partitions()), self._master.debug)
        debug_print("[SparkDriver]*****************",self._master.debug)
        self.args=args
        ...
        tasks.sort(lambda x,y: cmp(x.task_id, y.task_id))
        for task in tasks:
    ...
    def gen_stage_tasks(self, start_rdd, last_rdd, cur_stage_id):
        ...
        """
        debug_print("[SparkDriver] Generating Task for Stage {0} for RDD {1}".format(cur_stage_id, last_rdd), self._master.debug)
                            ...
                            if int(elem.split('_')[1]) == cur_par_id:
                                ...
                                for task in self.task_list.keys():
                                    if task.last_rdd.id == parent_rdd.id and elem.split('_')[0] == task.task_id.split('_')[1]:
                                        debug_print("[SparkDriver]Find Shuffle Parent Task {0} For Task ".format(task.task_id, "{0}_{1}".format(cur_stage_id, cur_par_id)))
