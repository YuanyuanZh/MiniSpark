...
#        self.task_node_table.__delitem__(worker_id)
    ...
    def gen_stage_tasks(self, start_rdd, last_rdd, cur_stage_id):
        ...
        """
        debug_print("[SparkDriver] Generating Task for Stage {0} for RDD {1}".fmat(cur_stage_id, last_rdd.id), self._master.debug)
    ...
    def get_result(self, task, part_id):
        result= self._master.get_rdd_result(task, part_id)
